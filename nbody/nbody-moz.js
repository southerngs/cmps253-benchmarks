/* The Computer Language Benchmarks Game
   ...
   Optimized by Roy Williams, Stefan Krause */

var SIZE = 5;
var bodyBuffer = new Float64Array( 7 * 5);
...
NBodySystem.prototype.advance = function(dt){
   ...
   var m1, m2;
	...
	var c = SIZE*7;
   for (var ofs_i=0; ofs_i<c; ofs_i+=7) {
      imass = bodyBuffer[ofs_i +6]
      for (var ofs_j=ofs_i+7; ofs_j<c; ofs_j+=7) {
         jmass = bodyBuffer[ofs_j +6];
		 ...
		 m1 = jmass * mag;
         bodyBuffer[ofs_i +3] -= dx * m1;
         bodyBuffer[ofs_i +4] -= dy * m1;
         bodyBuffer[ofs_i +5] -= dz * m1;

		 m2 = imass * mag;
         bodyBuffer[ofs_j +3] += dx * m2;
         bodyBuffer[ofs_j +4] += dy * m2;
         bodyBuffer[ofs_j +5] += dz * m2;
      }
      bodyBuffer[ofs_i +0] += dt * bodyBuffer[ofs_i +3];
      bodyBuffer[ofs_i +1] += dt * bodyBuffer[ofs_i +4];
      bodyBuffer[ofs_i +2] += dt * bodyBuffer[ofs_i +5];
   }
}
